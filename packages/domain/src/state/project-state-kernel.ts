 ...
 * (task 4.2). It owns: the append-only event history (the 4.1 runtime),
 * the replay-built projection, optimistic concurrency, the human-only
 ...
 *   fact set. R0 ruling (design.md, open questions): a global constant,
 *   not per-project config; per-project budgets are a later change.
 ...
 *   lifecycle table). Invariant rule 15, accepted at 90 days.
 * STATE_EVENT_SCHEMA_VERSION: the envelope schema_version the kernel
 *   stamps on every appended event (task 4.2: the event-envelope
 *   schema_version).
 */
export const EQUIP_SIZE_BUDGET = 64 * 1024; // serialized UTF-8 bytes
export const COMPETITIVE_GRACE_PERIOD_DAYS = 90; // invariant rule 15
...
function applyEvent(d: MutableProjection, e: StateEvent): void {
  ...
  switch (e.type) {
    ...
    case 'checkpoint.created': {
      // Standalone checkpoints are not a command in R0 — the redirect
...
export class ProjectStateKernel {
   ...
   * (invariant rule 15); archived→purged requires the double condition
   ...
   * records the verdict and keeps the candidate (the OQ-45 delivery
   * refinement hooks here).
   */
