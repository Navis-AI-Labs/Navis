 ...
 * builds on. It validates each event against the StateEvent schema,
 * validates seq contiguity (each seq is exactly head+1), deep-freezes on
 * append, and offers NO update or delete path (spec: no API may offer
 * update or delete on appended events).
 *
 * Sequencing and state_version assignment belong to the kernel; this type
 * only refuses events that do not continue the sequence.
 */
