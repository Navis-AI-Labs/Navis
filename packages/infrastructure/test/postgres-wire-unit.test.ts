...
describe('PostgresEventStore against a scripted wire (unit, no database)', () => {
  ...
  it('rejects a legacy snapshot row whose state arrived null instead of degrading', async () => {
    const projectId = uuidv7();
    const fake = makeFakeSql({
...
describe('runMigrations against a scripted wire (unit, no database)', () => {
  ...
  it('adopts a legacy checksum-less row instead of failing every later run', async () => {
    const legacy = makeFakeSql({ appliedRows: [{ version: '001_events', checksum: null }] });
    await runMigrations(legacy.sql);
    expect(legacy.beginCount()).toBe(0);
    const update = legacy.queries.find((q) =>
      q.text.includes('UPDATE schema_migrations SET checksum'),
    );
    expect(update).toBeDefined();
    expect(update?.params?.[0]).toMatch(/^[0-9a-f]{64}$/);
  });
});
