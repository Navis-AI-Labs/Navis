...
> **Capability intent** — 账本的信任保证必须活得比任何一家数据库厂商的选择更久。这一层把存储定义为可替换基建：领域代码面向端口，Postgres 线协议为主实现，内存实现保测试；团队明天选 Supabase、后天自部署，同一份领域代码不改一行。
> **Scope boundary** — 本能力只定义：EventStore 端口契约、Postgres 线协议适配器、内存适配器、迁移的纯 SQL 形态。**不含**：ORM 与查询构建器（明确排除）、平台 SDK 与平台鉴权（明确排除——RLS 可作为部署侧纵深防御，但没有任何行为可依赖它）、SQLite（不准入，内存适配器覆盖测试）、缓存/队列/对象存储（各自后续变更）。
