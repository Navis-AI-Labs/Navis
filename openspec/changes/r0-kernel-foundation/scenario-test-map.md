...
| 4.2  | `src/state/project-state-kernel.ts`（26 型封闭事件词表、仅追加 ProjectStateKernel：乐观并发 expected_version 门、State-material 仅边界/状态、人类专属+理由门、项目生命周期含归档级联、装备派生契约+64KiB 预算诊断、整体拒绝回程+吸收、四序交付门+开放尝试+attempt_no、保持 ai-proposes-human-enacts、重放时戳 updated_*、墓碑读侧、`src/errors/kernel.ts` 9 令牌封闭注册表、90 天宽限常数） | `test/state-kernel.test.ts` 47 用例（26 覆盖矩阵用例＋守卫先行矩阵＋奇异重放＋全字段矩阵） |
...
| 6.1  | `pnpm validate` 七道门〔更新：双模式全绿——带 DATABASE_URL 214/214；无库 213＋1 skip，覆盖率满格（语句/函数/行 100%，分支 >97%）〕                                                                                                                                                                                                                                                           | 214/214 带真库；213＋1 skip 无库                                                           |
...
## project-state-kernel spec 场景覆盖表（4.2）

| Spec 场景                                                                                              | 测试（`test/state-kernel.test.ts`）                                                                                                                                                                  |
| ------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 错误码封闭注册表：所有拒绝经注册表发出，URN=kernel/<token>                                             | 全套 expectErr 断言 code+urn（forbidden/rationale-required/version-conflict/equip-budget-exceeded/unaccepted-artifact/blocking-hold/unknown-effect-unclosed/project-not-active/open-attempt-exists） |
| 事件史仅追加且结构不可变                                                                               | `no update/delete surface`（表面扫描＋冻结断言）；T17b `stale and divergent expected versions both fail … log unchanged`                                                                             |
| 重放构建版本化投影（canonical JSON 恒等）                                                              | `200 events` 重放同一性＋`fromEvents` 重建＋verifyIntegrity（腐坏日志/间隙日志/篡改投影三向探针）                                                                                                    |
| 边界版本化：人类专属、理由门、零污染、装备整体失效                                                     | `agent update is forbidden with zero pollution`、`human update advances version exactly 1 and marks equips stale`、`redirect_work … without the version`                                             |
| 项目生命周期：人类门控、带理由、非破坏（暂停锁四族、完成拒于阻塞保持、归档级联带 cause、终态不可重启） | lifecycle describe 块 5 用例                                                                                                                                                                         |
| 装备派生契约：verified_facts=活跃+项目域、预算 64KiB 显式失败+诊断事件                                 | `verified_facts are exactly the project-scope active assets`、`budget overflow fails explicitly with a diagnostics event`                                                                            |
| 交付门序：not-active→unaccepted→blocking-hold→unknown-effect；单开放尝试                               | `gate order` 矩阵＋`one open attempt per (asset, target)`＋missing-sha256/未知资产负例                                                                                                               |
| 保持 ai-proposes-human-enacts：agent 注册不阻塞、事件携带 actor、重激活人类专属+理由                   | `agent hold does not block delivery until a human confirms it`、`every hold event carries the acting participant id`、`agent reactivation is forbidden…`                                             |
...
内核状态与操作层（4.1/4.2）已随本轮交付完成。WorkRun 状态机、向量时钟、Effect Ledger 行为层、快照策略、schema 层 Interfaces/Link Types/Type Registry、submission_criteria 框架——均已在 proposal.md/design.md/tasks.md 声明为后续变更。端口与适配器（4.3/5.x）已随本次交付完成。
