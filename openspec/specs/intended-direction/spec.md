# intended-direction Specification

## Purpose

> **Capability intent** — Where a project goes next is a first-class record, not chat memory. Any participant proposes an intended direction; only a human resolves it, with a mandatory rationale; resolution is terminal and immutable. Direction facts are the third time-plane: they never advance the project's state version and never mutate Current State directly — a confirmed direction becomes material only through subsequent explicit commands.
> **Scope boundary** — This capability defines only: the direction record lifecycle (propose, resolve, terminal immutability) and its query semantics (filter views over projected records) within a single project aggregate. Not included: turning a confirmed direction into Work or other material state (the receiving commands own that), transport or UI surfaces (contracts capability), cross-project direction flows, and the authorization system beyond the kernel's human-role checks.
